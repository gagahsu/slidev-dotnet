...
public class ProductsPageTests(EShopWebFactory factory)
    : IClassFixture<EShopWebFactory>
{
    ...
    {
        var html = await _client.GetHtmlAsync("/Customer/Home/Details/3");
    ...
    {
        var response = await _client.GetAsync("/Customer/Home/Details/99");
