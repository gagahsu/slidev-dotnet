...
public class PriceCalculatorTests
{
    ...
    [Fact]
    public void Stock_小於零_驗證失敗()
    {
        var bean = new Product { Name = "西達摩", Origin = "衣索比亞", Price = 420, Stock = -1 };
        var results = new List<ValidationResult>();

        var valid = Validator.TryValidateObject(
            bean, new ValidationContext(bean), results, validateAllProperties: true);
