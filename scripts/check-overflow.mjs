...
// CHROMIUM_PATH 可指定瀏覽器位置（例如 /opt/pw-browsers/chromium）
...
for (let n = from; n <= Math.min(total, to); n++) {
  ...
  if (r && r.wrapped > 0) wrappedPages++
  if (r && r.wrapped > 0) console.log(`第 ${n} 頁 有 ${r.wrapped} 行程式碼過長而換行：${r.title}`)
}
console.log(`檢查 ${from}～${Math.min(total, to)} 頁，${bad} 頁超出版面，${wrappedPages} 頁有程式碼換行`)
await browser.close()
if (bad || wrappedPages) process.exit(1)
